...
const ReportsDomesticMachine = () => {
  ...
  const searchClickHandler = () => {
    ...
    if (userId) {
      filteredRows = filteredRows.filter((row) =>
        row.transporter.toLowerCase().includes(userId.toLowerCase())
      );
    }
    if (charge) {
      filteredRows = filteredRows.filter((row) =>
  ...
  useEffect(() => {
    const getDomestic = async () => {
      try {
        const response = await getAPI(apiEndpoints.getAllInvoiceDataReportsDM);
  ...
  return (
    <div className="mt-3">
      ...
      <CustomTable
        divClass={"table-container h-470px mt-4"}
        checkbox={false}
        // rows={rows}
        rows={filterRows}
        columns={columns}
...
export default ReportsDomesticMachine;
