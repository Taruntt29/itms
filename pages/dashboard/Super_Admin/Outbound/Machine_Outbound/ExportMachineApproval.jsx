...

    {
      ...
      renderCell: (param) => (
        <span
          ...
          onClick={() => {
            const user = allUsers.find((itm) => itm.id === param.row.id);
            outletContext.setActiveTab(29);
    ...
    const getExport = async () => {
      try {
        const response = await getAPI(
          `${apiEndpoints.getInvoicesByStatusExports}forApproval`
        );
...
export default ExportMachineApproval;
