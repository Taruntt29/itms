...

    {
      ...
      renderCell: (param) => (
        <span
          ...
          onClick={() => {
            const exort = filterRows.filter(
              (data) =>
                data.transporter === param.row.transporter ||
                data.date === param.row.date
            );

            outletContext.setActiveTab(25);
            outletContext.setOutbound(exort);
          }}
        >
    ...
    const getExport = async () => {
      try {
        const response = await getAPI(
          `${apiEndpoints.getInvoicesByStatusExports}fresh`
        );
