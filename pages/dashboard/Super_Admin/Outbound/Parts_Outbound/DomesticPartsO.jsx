    ...
    { field: "id", headerName: "ID", width: 70 },
    {
      ...
      renderCell: (param) => (
        <span
          ...
          onClick={() => {
            const shyam = filterRows.filter(
              (data) =>
                data.transporter === param.row.transporter ||
                data.date === param.row.date
            );

            outletContext.setActiveTab(30);
            outletContext.setOutbound(shyam);
          }}
        >
    ...
    const getParts = async () => {
      try {
        const response = await getAPI(
          `${apiEndpoints.getInvoicesByStatusParts}fresh`
        );
